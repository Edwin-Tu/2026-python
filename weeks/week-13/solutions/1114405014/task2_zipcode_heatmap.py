...
def get_top_counties(all_years: dict[int, dict], top_n: int = 10) -> list[str]:
    ...
    sorted_counties = sorted(totals, key=totals.get, reverse=True)
...
def main():
    ...
    year_data = {y: load_county_counts(y, data_dir) for y in years}
    top_counties = get_top_counties(year_data, top_n=10)

    heatmap_data = []
    for county in top_counties:
        heatmap_data.append([year_data[y].get(county, 0) for y in years])
    ...
    print("Task 2 圖表已儲存至 output/task2.png")
