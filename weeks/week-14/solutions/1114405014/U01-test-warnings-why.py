...
class WhySkip(unittest.TestCase):
    ...
    def test_match_case(self):
        """match statement 需要 Python 3.10 以上，在舊版環境中自動跳過，
        避免因語法錯誤導致整個測試套件中斷"""
        x = 1
        match x:
            case 1:
    ...
    def test_mac_only(self):
        """只在 macOS 平台執行的測試：若在 Linux 或 Windows 上跑，
        /Users 目錄可能不存在，使用 skipUnless 可避免誤判為測試失敗"""
        import os
...
def old_api_bad(x):
    ...
    當使用者呼叫 old_api_bad(1)，警告會指向此函式內部（第 72 行），
    使用者無法直接找到自己程式中應該修改的位置"""
...
def old_api_good(x):
    ...
    設定 stacklevel=2 之後，警告會指向呼叫此函式的那一行。
    對於函式庫開發者來說，這是最基本的使用者體驗要求"""
