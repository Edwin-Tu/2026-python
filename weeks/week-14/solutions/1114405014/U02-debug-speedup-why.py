...
def low_level():
    """模擬底層函式拋出 ValueError 的情境"""
    raise ValueError("低階：值不對")


def variant_a():
    """raise X from e：明確標示「因為 e 所以 X」（推薦用於包裝底層錯誤）。
    使用 from 關鍵字可以建立例外鏈（exception chain），
    traceback 會同時顯示原始例外與包裝後的例外，方便除錯時追根溯源"""
    try:
        low_level()
...
def variant_b():
    ...
    不使用 from 時 Python 仍會自動附加 __context__，
    但在某些情形下鏈結可能不明確，建議優先使用 explicit 'from'"""
    try:
        low_level()
    except ValueError:
...
def variant_c_good():
    ...
    這是最安全的重新拋出方式，因為執行堆疊（stack frame）完全保留，
    上層除錯時可以看到原始出錯位置"""
    try:
        low_level()
    except ValueError:
...
def variant_c_bad():
    ...
    因為 raise e 會重新觸發例外，但 traceback 從目前的堆疊位置開始，
    原始出錯的那一行資訊就遺失了，大幅增加除錯難度"""
    try:
        low_level()
    except ValueError as e:
        raise e
...
def slow_version(items):
    ...
    Python 在函式內部存取全域變數需要經過全域命名空間查詢，
    且 math.sqrt 還需額外的屬性查找（attribute lookup），成本較高"""
    result = []
    for x in items:
...
def fast_version(items):
    ...
    將全域函式繫結為區域變數後，Python 使用 LOAD_FAST 指令（區域變數存取），
    速度遠快於 LOAD_GLOBAL + LOAD_ATTR 的組合"""
    sqrt = math.sqrt
